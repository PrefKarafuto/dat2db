...
$year        = isset($_GET['year'])        ? intval($_GET['year'])              : 0;  // 検索する年（整数）
$limit       = 20;
$offset      = ($page - 1) * $limit;
...
// 全掲示板情報を1回のクエリで取得（カテゴリID・カテゴリ名付き）
$allBoards = [];
$board_sql = "SELECT board_id, board_name, category_id, category_name FROM Boards ORDER BY board_name ASC";
$board_result = $db->query($board_sql);
while ($row = $board_result->fetchArray(SQLITE3_ASSOC)) {
    $allBoards[] = $row;
}

// カテゴリ選択用：カテゴリID => カテゴリ名 の連想配列を作成
$categories_map = [];
foreach ($allBoards as $b) {
    if (!isset($categories_map[$b['category_id']])) {
        $categories_map[$b['category_id']] = $b['category_name'];
    }
}
ksort($categories_map);

// 検索実行前の初期化
$results = [];
$total_results = 0;
$total_pages = 0; // 検索未実施の場合にも定義される
...
if ($search_query !== '') {
    ...
    // FTS用検索クエリの作成（各検索タイプに応じ、前方一致用にアスタリスクを付与）
    switch ($search_type) {
        case 'message':
    ...
    if ($scope === 'board') {
        ...
        // カテゴリ指定はカテゴリID で絞る
        $sql .= " AND Boards.category_id = :category";
        $params[':category'] = $category;
    }
    
    // 追加：年指定（スレッド建て日時＝thread_idがUnixタイムスタンプと仮定）
    if ($year > 0) {
    ...
    foreach ($params as $key => $value) {
        if (in_array($key, [':start_ts', ':end_ts'])) {
...
<body>
                ...
                <div class="form-group">
                    <select name="category" id="category">
                        ...
                        <?php foreach ($categories_map as $cat_id => $cat_name): ?>
                            <option value="<?php echo escape_html($cat_id); ?>" <?php if ($category === $cat_id) echo 'selected'; ?>>
                                <?php echo escape_html($cat_name); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
    ...
    <footer>
        <h2><a href="https://github.com/PrefKarafuto/dat2db">dat2db</a> Ver.0.2</h2>
    </footer>
    <!-- script.js を外部読み込み -->
    <script>
    document.addEventListener('DOMContentLoaded', function(){
      ...
      // 初期表示時、GET パラメータで選択済みの掲示板があれば保持
      window.initialBoard = boardSelect.getAttribute('data-initial') || '';

      function updateBoardSelect() {
          var selectedCategory = categorySelect.value;
          // 既存の選択肢をクリア
          boardSelect.innerHTML = '<option value="">掲示板を選択</option>';
          
          allBoards.forEach(function(b){
              // カテゴリ比較は category_id で行う
              if (selectedCategory === '' || b.category_id === selectedCategory) {
