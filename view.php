...
function handleWebRequest($db, $base_url, $path_segments) {
    ...
    if (empty($path_segments)) {
        ...
        } elseif ($board_id === 'bbsmenu.json') {
            displayBBSmenuJson($db, $base_url);
        } else  {
            if (!isValidBoardId($board_id)) {
                exitWithError("無効なboard_idです。");
            } else {
...
// 掲示板一覧の表示（カテゴリごとにグループ化）
function displayBoardList($db, $base_url) {
    $result = $db->query("SELECT board_id, board_name, category_name FROM Boards ORDER BY category_name ASC, board_name ASC");
    if (!$result) exitWithError("データベースエラーが発生しました。");

    $boards_by_category = [];
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        $cat = $row['category_name'] ?: '未分類';
        if (!isset($boards_by_category[$cat])) {
            $boards_by_category[$cat] = [];
        }
        $boards_by_category[$cat][] = $row;
    }
    ...
    foreach ($boards_by_category as $category => $boards) {
        $category_escaped = htmlspecialchars($category, ENT_QUOTES, 'UTF-8');
        echo "<h2>{$category_escaped}</h2>";
        echo "<ul>";
...
// BBSMENUの表示（専ブラ用、カテゴリごとにグループ化済み）
function displayBBSmenuHtml($db, $base_url) {
    ...
    echo "<!DOCTYPE html>";
    echo "<html lang='ja'>";
    echo "<head><meta charset='Shift_JIS'><title>BBS MENU</title>";
    echo "<link rel='stylesheet' href='" . dirname($_SERVER['SCRIPT_NAME']) . "/thread.css'>";
    echo "<BASE TARGET=\"_blank\"></head>";
    echo "<body>";
    echo "<b>BBS MENU</b><br><br><font size=2>";
    ...
    $html_output_sjis = mb_convert_encoding($html_output, 'SJIS', 'UTF-8');
    echo $html_output_sjis;
}
// --- displayBBSmenuJson 関数 ---
function displayBBSmenuJson($db, $base_url) {
    // Boardsテーブルから全掲示板情報を取得
    $result = $db->query("SELECT board_id, board_name, category_name FROM Boards ORDER BY category_name ASC, board_id ASC");
    if (!$result) exitWithError("データベースエラーが発生しました。");
    ...
    foreach ($categories as $category_name => $boards) {
        ...
        $menu_list[] = array(
            ...
            "category_name" => $category_name,
            "category_number" => (string)$cat_number,
            "category_content" => $category_content
        );
        $cat_number++;
    }
    
    // last_modify: 現在のタイムスタンプを利用
    $last_modify = time();
    $last_modify_string = date("Y/m/d(D) H:i:s", $last_modify);
    
    $json_array = array(
        "last_modify" => $last_modify,
        "last_modify_string" => $last_modify_string,
        "menu_list" => $menu_list
    );
...
function displayAllResponses($db, $board_id, $thread_id, $base_url) {
    ...
    // 絶対リンクで CSS を読み込む（同一ディレクトリにある thread.css）
    $cssUrl = dirname($_SERVER['SCRIPT_NAME']) . '/thread.css';
    ...
    echo "<a href='{$base_url}'>←← 掲示板一覧に戻る</a> <a href='{$base_url}/{$board_id}/'>← スレッド一覧に戻る</a>";
...
function displaySelectedResponses($db, $board_id, $thread_id, $response_format, $base_url) {
    ...
    $result_title = $stmt_title->execute();
    if (!$result_title) {
        exitWithError("スレッドタイトル取得中にデータベースエラーが発生しました。");
    }
    $row = $result_title->fetchArray(SQLITE3_ASSOC);
    if ($row) {
        $res_count = $row['response_count'];
    ...
    foreach ($post_orders as $index => $order) {
        $stmt->bindValue($index + 3, $order, SQLITE3_INTEGER);
    }
    $result = $stmt->execute();
    if (!$result) {
        exitWithError("レス取得中にデータベースエラーが発生しました。");
    }
    $cssUrl = dirname($_SERVER['SCRIPT_NAME']) . '/thread.css';
    ob_start();
    echo "<!DOCTYPE html>";
    ...
    echo "<body>";
    echo "<div class='container'>";
    echo "<h1>" . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . " ($res_count)</h1>";
    echo "<a href='" . htmlspecialchars($base_url, ENT_QUOTES, 'UTF-8') . "'>←← 掲示板一覧に戻る</a> ";
    echo "<a href='" . htmlspecialchars($base_url . '/' . $board_id . '/', ENT_QUOTES, 'UTF-8') . "'>← スレッド一覧に戻る</a>";
    $hasResult = false;
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        $hasResult = true;
        displayResponse($row, $base_url, $board_id, $thread_id);
